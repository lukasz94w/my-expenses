...
public class OuterPieChartDrawer {
    ...
    private void configurePieChartAppearance() {
        ...
        pieChart.setId(idOfOuterPieChart);

        RelativeLayout.LayoutParams layoutParamsForBarOuterPieChart = new RelativeLayout.LayoutParams(dpToPx(310), dpToPx(310));
        layoutParamsForBarOuterPieChart.addRule(RelativeLayout.CENTER_HORIZONTAL);
        layoutParamsForBarOuterPieChart.setMargins(0, dpToPx(10), 0, 0);
        pieChart.setLayoutParams(layoutParamsForBarOuterPieChart);
    }
    ...
    private void preparePieChartDataAndView() {
        ...
        Map<String, Integer> map = monthTransactions.stream().
                collect(groupingBy(Transaction::getCategory, LinkedHashMap::new, summingInt(Transaction::getAmount)));
